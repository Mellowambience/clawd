...
const ALLOWED_PARENT_ORIGINS = (process.env.EXPO_PUBLIC_PARENT_ORIGINS || "")
  .split(",")
...
function isAllowedParentOrigin(origin: string): boolean {
  ...
  if (ALLOWED_PARENT_ORIGINS.length === 0) return true;
  return ALLOWED_PARENT_ORIGINS.includes(origin);
}

function getTargetOrigin(): string {
  return ALLOWED_PARENT_ORIGINS.length === 1 ? ALLOWED_PARENT_ORIGINS[0] : "*";
}
