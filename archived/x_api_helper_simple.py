    ...
    if creds:
        ...
        if client:
            try:
                me = client.get_me()
                if me.data:
                    print(f"[SUCCESS] Successfully connected to X API as @{me.data.username}")
                else:
                    print("[ERROR] Could not retrieve user information")
            except Exception as e:
                print(f"[ERROR] Connection test failed: {e}")
        else:
            print("[ERROR] Could not create X API client")
    else:
        print("\n[ERROR] Could not load credentials. Please check your configuration.")